...
def getTGC(alpha0, propDist):
    ...
    defined as alphaDB = 20/x*log10[p(0)/p(x)] where x is the propagation 
    distance, p(0) is the incident pressure, p(x) is the spatially variant 
    pressure.  As a result, p(x)/p(0) = 10^(-alphadB*x/20). Additionally, 
    alpha is modeled as alphaDB = alpha0*f^n where f is frequency and 0 < n < 1
    . For tissue, n ~ 1. alpha0 is usually the parameter specified in units of 
    dB/(MHz-cm). We can compensate therefore by multiplying each A-line by 
    10^(alpha0*f*propDist*100/20). Note that this does not take into account
...
def main():
    ...
    # load data from file, [sensorData] = 96x32x1585 -> transmission x recording element x time index
    sensorData = sio.loadmat('example_us_bmode_sensor_data.mat')['sensor_data']
    ...
    for r in range(len(images)):
        ...
        for n in range(numTxBeams):
            imTrunc[n, :] = envDet(imTrunc[n, :], 2*zTrunc/c0,
                                   method='hilbert')
